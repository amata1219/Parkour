			...
			for(int slotIndex = 0; slotIndex < parkourList.size(); slotIndex++){
				...
				l.put((s) -> {
					...
					s.onClick((event) -> {
						...
						//表示例: TP to 1 @ Update1!
						event.player.spigot().sendMessage(ChatMessageType.ACTION_BAR, new TextComponent(TextTemplate.apply("$0TP to $1 @ $2!", ChatColor.GRAY, size, parkourName)));
					});

					s.icon(Material.GRASS_BLOCK, (i) -> {
						//表示例: 1 @ Update1
						i.displayName = TextTemplate.apply("$0$1 @ $2", ChatColor.AQUA, size, parkourName);
